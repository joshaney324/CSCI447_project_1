...
def plot_boxplot(ori_data, noisy_data, filename):
    ...
    """
    # create two subplots with a big, overall heading
    fig, ax_list = plt.subplots(1, 2, tight_layout=True)
    fig.suptitle(filename, fontsize="x-large")

    # create 2 boxplot diagrams in seperate subplots
    ...
    # adjust y-scaling for a cleaner representation 
...
def plot_confusion_matrix(matrix, filename, labels):
    ...
    """
    # display matrix data on a 2D regular raster
    fig = plt.figure()
    plt.imshow(matrix, cmap="Blues")

    # relabel y- and x-axes
...
def plot_avgs(avg_data, filename):
    ...
    # string arrays for individual labeling of the datasets
    ...
    # plotting a bar chart for each dataset's precision, recall and accuracy values
    ...
    # adding labels, title, tick steps and legend entries
